                          ...
                          bool verbose)
    {
        ...
        try
        {
            ...
            for (const auto &[addressKey, addressNode] : config)
            {
                ...
                if (!addressTable)
                {
                    spdlog::error("Invalid settings: top-level key '{}' must contain a table", std::string(addressKey.str()));
                    return false;
                }
                ...
                visitTable = [&](const toml::table &tbl, const std::string &scopePath) -> bool
                {
                    for (const auto &[key, node] : tbl)
                    {
                        ...
                        if (scopePath.empty())
                        {
                            spdlog::error("Invalid settings in address '{}': values must be under [\"<address>\".par] or channel sections", address);
                            return false;
                        }
                        ...
                        {
                            spdlog::error("Invalid value type for {}.{} in address '{}'", scopePath, std::string(key.str()), address);
                            return false;
                        }
...
int main(int argc, char **argv)
{
    ...
    std::map<std::string, std::vector<std::pair<std::string, std::string>>> settingsByAddress;
    std::map<std::string, std::vector<std::pair<std::string, std::string>>> channelDefaultsByAddress;
    if (!options.settingsFile.empty() && !loadSettingsFile(options.settingsFile, options.address, settingsByAddress, channelDefaultsByAddress, options.verbose))
    {
        return 1;
    }
