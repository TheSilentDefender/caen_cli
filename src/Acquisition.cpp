...
Acquisition::Acquisition(CaenScope &scope,
                         ...
                         std::string runToken,
                         const bool verbose)
    : scope_(scope), outputDirectory_(std::move(outputDirectory)), runToken_(std::move(runToken)),
      verbosePrefix_(
          "[verbose][" + (scope_.getAddress().empty() ? std::string("unknown") : scope_.getAddress()) + "] "),
      verbose_(verbose) {
}
...
Acquisition::LatestEventSnapshotPtr Acquisition::getLatestEventSnapshot() const {
...
void Acquisition::start() {
    ...
    acquisitionThread_ = std::thread([this] { acquisitionLoop(); });
    savingThread_ = std::thread([this] { savingLoop(); });
