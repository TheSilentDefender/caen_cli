        ...
        nlohmann::json format = nlohmann::json::array(
            {
                ...
                {{"name", "TIMESTAMP"}, {"type", "U64"}},
                {
                    ...
                    {"shape", nlohmann::json::array({nCh, recordLength})}
                },
...
bool Acquisition::readDeviceConfiguration() {
    try {
        ...
        if (useSoftwareTrigger_ && startSource.find("SWcmd") == std::string::npos) {
            spdlog::warn(
                "acqtriggersource contains SwTrg but /par/startsource=\"{}\" does not include SWcmd; software triggers may not start acquisition.",
                startSource);
        }
