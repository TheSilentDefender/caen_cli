...
{
  return writeChecked(f, &packet.trigId, sizeof(uint32_t), 1) && writeChecked(f, &packet.timestamp, sizeof(uint64_t), 1) && writeChecked(f, &samplesPerChannel, sizeof(uint32_t), 1) && writeChecked(f, &kTimeResolutionNs, sizeof(uint64_t), 1) && writeChecked(f, &channelsWithData, sizeof(int32_t), 1);
}
                                      ...
                                      uint64_t &eventBytes)
{
  for (int ch = 0; ch < nChannels_; ++ch)
  {
    ...
    eventBytes += sizeof(float) * writeSamples;
  }
...
void Acquisition::savingLoop()
{
  ...
  while (true)
  {
...

    {
      ...
      queueCvNotEmpty_.wait(lock, [this]
                            { return stopRequested_ || producerDone_ || !eventQueue_.empty(); });
